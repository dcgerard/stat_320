...
- **Exercise**: Calculate this in R (*hint*: `pi` is $\pi$ in R)
    $$
...
- **Exercise**: Create the following vector $x = (1, -2, 1.3)$. What is the mean and standard deviation of this variable?
...
- **Exercise**: Create a vector from 1 to 1000, take ths square root of each element, then sum them up.
...
- **Exercise**: What does the `by` argument do in `seq()`? Try it out.

```{r}
#| eval: false
#| echo: false
# increments of 2 instead of 1.
seq(1, 10, by = 2)
```
