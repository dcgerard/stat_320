...
## Solution
Let $X_i$ be the total heart weight for normal male $i$. Let $Y_j$ be the total heart weight for diseased male $j$. Then we assume
\begin{align*}
X_i &\sim N(\mu_1, \sigma_1^2)\\
Y_j &\sim N(\mu_2, \sigma_2^2)
\end{align*}
We want to test $H_0: \mu_1 = \mu_2$ versus $H_A: \mu_1 \neq \mu_2$. We have the following summary statistics:
```{r}
heart |>
...
## Solution
Let $X_i$ be the body weight for normal male $i$. Let $Y_j$ be the body weight for diseased male $j$. Then we assume
\begin{align*}
...
## Solution
Let $X_i$ be the pretest questionnaire value for experimental individual $i$. Let $Y_j$ be the pretest questionnaire value for control individual $j$. Then we assume
\begin{align*}
X_i &\sim N(\mu_1, \sigma^2)\\
Y_j &\sim N(\mu_2, \sigma^2)
\end{align*}
...
## Solution
Let $X_i$ be the posttest questionnaire value for experimental individual $i$. Let $Y_j$ be the posttest questionnaire value for control individual $j$. Then we assume
\begin{align*}
X_i &\sim N(\mu_1, \sigma^2)\\
Y_j &\sim N(\mu_2, \sigma^2)
\end{align*}
...
## Solution
Let $X_i$ be the serum cholesterol for oral contraceptive user $i$. Let $Y_j$ be the serum cholesterol for nonuser $j$. Then we assume
\begin{align*}
X_i &\sim N(\mu_1, \sigma^2)\\
Y_j &\sim N(\mu_2, \sigma^2)
\end{align*}
...
## Solution
Let $X_i$ be the systolic blood pressure individual $i$ in the no alcohol use group. Let $Y_j$ be the systolc blood pressure for individual $j$ in the low alcohol use group. Then we assume
\begin{align*}
X_i &\sim N(\mu_1, \sigma^2)\\
Y_j &\sim N(\mu_2, \sigma^2)
\end{align*}
...
## Solution
Let $X_i$ be the FEV at baseline for individual $i$ in the smoking. Let $Y_j$ be the FEV at baseline for individual $j$ in the non-smoking group. Then we assume
\begin{align*}
X_i &\sim N(\mu_1, \sigma^2)\\
Y_j &\sim N(\mu_2, \sigma^2)
\end{align*}
...
The rest of these are **very** similar to the above. I just didn't have time to finish them in detail. But they are all just deciding to use either a paired or two-sample $t$-test, implementing the test using the critical value or the $p$-value method, and stating the conclusion. Sometimes they ask you for a confidence interval, or to calculate the power or sample size. All of these have been done above in my solutions. Below are the numerical soltuions, not the details. Keep in mind:

- Never first test if the variances are equal
- Prefer calculating the $p$-value, and understanding what tail area we are calculating.
- Never use "accept" when we fail to reject the null.
- Use R functions, not look-up tables, to calculate the $p$-values.


```{r}
#| echo: false

#' @param type two-sample or paired t-test
#' @param xbar The mean of the difference, or the mean of group 1
#' @param ybar The mean of group 2
#' @param sx The standard deviation of group 1, or the standard deviation of the difference
#' @param sy The standard deviation of group 2
#' @param nx The sample size of group 1, or of the total sample
#' @param ny The sample size of group 2
#' @param equal_var Assume equal variances?
#' @param alternative Two-sided test or one-sided (greater/less)
#' @param df The degrees of freedom if equal_var = FALSE
#' @param level The confidence level
#' @param alpha The significance level of the test.
t_results <- function(
    type = c("two_sample", "paired"), 
    xbar = NULL, 
    ybar = NULL, 
    sx = NULL, 
    sy = NULL, 
  ...
  stopifnot(!is.null(xbar), !is.null(sx), !is.null(nx))
  if (type == "two.sample") {
    stopifnot(!is.null(ybar), !is.null(sy), !is.null(ny))
  }
  if (!equal_var & type == "two_sample") {
    stopifnot(!is.null(df))
  }
  
  if (type == "two_sample" && equal_var) {
    s_pooled <- sqrt(((nx - 1) * sx^2 + (ny - 1) * sy^2) / (nx + ny - 2))
    cat("Pooled Standard Deviation:", s_pooled, "\n")
  }
  ...
  ## plot and p-value and critical value --
  mval <- max(ceiling(abs(tstat)), 3)
  if (alternative == "greater") {
    pl <- plt_t(lb = tstat, rng = c(-mval, mval), df = df, two_sided = FALSE) +
      geom_vline(xintercept = tstat, linetype = 2)
    pval <- 1 - pt(q = tstat, df = df)
    crit <- qt(p = 1 - alpha, df = df)
    
    if (tstat > crit) {
      result <- "Reject since t > critical value"
    } else {
      result <- "Fail to reject since t < critical value"
    }
  } else if (alternative == "less") {
    pl <- plt_t(ub = tstat, rng = c(-mval, mval), df = df, two_sided = FALSE) +
      geom_vline(xintercept = tstat, linetype = 2)
    pval <- pt(q = tstat, df = df)
    crit <- pt(q = alpha, df = df)
    ...
    pl <- plt_t(lb = tstat, rng = c(-mval, mval), df = df, two_sided = TRUE) +
      geom_vline(xintercept = tstat, linetype = 2)
    ...
    pl <- plt_t(ub = tstat, rng = c(-mval, mval), df = df, two_sided = TRUE) +
      geom_vline(xintercept = tstat, linetype = 2)
    pval <- 2 * pt(-abs(tstat), df = df)
    crit <- qt(1 - alpha / 2, df = df)
  ...
  ## CI--
  mult <- qt(p = 1 - (1 - level)/2, df = df)
  ci <- c(effect - mult * se, effect + mult * se)
  
  ## Print everything --
  cat("T-statistic:", tstat, "\n")
  cat("Compare to t-distribution with", df, "degrees of freedom\n")
  cat("Critical Value:", crit, "\n")
  cat("At significance level", alpha, ",", result, "\n")
  print(pl)
  cat("P-value:", pval, "\n")
  cat(round(level * 100), "% CI: (", ci[[1]], ",", ci[[2]], ")\n")
}
```
...
```{r}
#| echo: false
t_results(type = "paired", xbar = -0.7, sx = 6.2, nx = 61)
```
...
```{r}
#| echo: false
t_results(type = "paired", xbar = -0.049, sx = 0.237, nx = 8)
```
...
```{r}
#| echo: false
t_results(type = "paired", xbar = -0.08, sx = 0.23, nx = 44, alternative = "two_sided")
```
