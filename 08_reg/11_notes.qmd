...
# Estriol vs Birthweight

-   For this lesson, we will use the study from @greene1963urinary exploring the association between the estriol level in pregnant women and birthweight.
...
-   To begin, we'll look at the association between these variables
    ...
    ```{r}
    #| fig-alt: "Scatterplot of estriol and birthweight."
...
#| fig-width: 8
#| fig-height: 2.5
#| fig-alt: "Line plots showing direction of lines based on sign of slope."
beta0 <- 1
beta1 <- 2
x1 <- -1
x2 <- 2
...
-   The distribution of $Y$ is conditional on the value of $X$
    ```{r}
    #| echo: false
    #| fig-alt: "Line with error distributions."
    pl1
    ```
...
-   Our $Y$ values are sampled from this distribution
    ```{r}
    #| echo: false
    #| fig-alt: "Line with error distributions and points."
    pl2
    ```
...
-   Large sum of squares of three points
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "Scatterplot with three points and boxes whose area are sums of squares."
    df <- tribble(
      ~x, ~y,
      0, 0,
      3, 3,
...
-   Bad Fit:
    ```{r, echo=FALSE}
    #| fig-alt: "Scatterplot with light and error lines from points to line."
    beta0 <- 20
    beta1 <- 1
...
-   We estimate it by the variability in the residuals.
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "Scatterplot with distances from lines."
    estriol |>
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "Scatter plot of residuals with distance from 0."
    estriol |>
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "Histogram of residuals."
    estriol |>
...
- Graphic: 
  ![](cartoons/linear_p.png){alt="Visualization of p-value in linear model."} \ 
...
1.  This is how you make the plot:
    ```{r}
    #| fig-alt: "Scatterplot of BAC on beers."
...
```{r}
#| echo: false
#| fig-alt: "Scatterplot of estriol and birthweight with residuals from y-bar."
estriol |>
...
```{r}
#| echo: false
#| fig-alt: "Scatterplot of birthweight on estriol with residuals from OLS line."
estriol |>
  ggplot() +
  geom_point(aes(x = estriol, y = birthweight)) +
...
```{r}
#| echo: false
#| fig-alt: "Scatterplot of birthweight on estriol with difference in residuals."
estriol |>
  ggplot() +
  geom_point(aes(x = estriol, y = birthweight)) +
...
```{r}
#| echo: false
#| fig-alt: "P-value of F-test is area to the right under density curve."
...
-   Interpolation
    ```{r}
    #| echo: false
    #| fig-alt: "Scatterplot showing interpolation."
...
-   Extrapolation
    ```{r}
    #| echo: false
    #| fig-alt: "Scatterplot showing extrapolation."
    ggplot(data = estriol, mapping = aes(x = estriol, y = birthweight)) +
      geom_point() +
      geom_abline(slope = coef(lmout)[2], intercept = coef(lmout)[1], lwd = 1, col = "blue", alpha = 1/2) +
...
```{r}
#| echo: false
#| fig-alt: "Curved residual plot with unequal variance."
lmout <- lm(y ~ x)
res_vec <- resid(lmout)
fit_vec <- fitted(lmout)
qplot(x, y, xlab = "X", ylab = "Y", main = "Raw Data")+ geom_smooth(se = FALSE, method = "lm")
...
```{r}
#| echo: false
#| fig-alt: "Unequal variance residual plot."
lmout <- lm(y ~ x)
res_vec <- resid(lmout)
fit_vec <- fitted(lmout)
qplot(x, y, xlab = "X", ylab = "Y", main = "Raw Data")+ geom_smooth(se = FALSE, method = "lm")
...
```{r}
#| fig-alt: "Scatterplot of birthweight on estriol with OLS line."
...
```{r}
#| fig-alt: "Residual plot with loess curve and point-wise confidence intervals."
lm(birthweight ~ estriol, data = estriol) |>
  augment() |>
...
Looks good. No curvilinear trends, equal variance looks fine.
```{r}
#| fig-alt: "Scatterplot of BAC on beers with OLS line."
ggplot(bac, aes(x = beers, y = bac)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE)
```
...
```{r}
#| fig-alt: "Residual plot with lowess curve and pointwise confidence intervals."
lm(bac ~ beers, data = bac) |>
  augment() |>
  ggplot(aes(x = .fitted, y = .resid)) +
  geom_point() +
...
```{r}
#| fig-alt: "Scatterplot of fev on height with OLS line."
...
```{r}
#| fig-alt: "Scatterplot of fev on height with quadratic OLS curve."
ggplot(fev, aes(x = height, y = fev)) +
  geom_point() +
...
#| fig-alt: "Scatterplot of species on area."
...
- The actual value of $Y_i$ varies about this point according to a normal distribution.
  ...
  ![](./cartoons/plane_me.png){alt="Visualization of multiple linear regression model."}\ 
...
-   Here is the relationship between `fat` and `midarm`
    ```{r}
    #| echo: false
    #| fig-alt: "Scatterplot of fat on midarm."
...
-   Here is the relationship between `fat` and `midarm` controlling for triceps
    ```{r}
    #| echo: false
    #| fig-alt: "Scatterplot of fat on midarm color-coded by triceps group."
    body |>
...
-   We compare our observed $t$ statistic to this null distribution to get a $p$-value
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is twice area to the right of absolute value of observed value."
...
-   Compare our observed $F$ statistic to this distribution to get a $p$-value
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is area to the right of observed F-statistic."
...
-   The variance of the residuals varies based on their $x$ values (closer to the mean has lower variance). So folks often prefer plotting the standardized residuals that adjustes fo this. They are standardized to all have variance 1.
    ```{r}
    #| fig-alt: "Standardized residual plot."
    lm(sbp ~ age + birthweight, data = bp) |>
      augment() |>
...
-   Don't try to make one manually, just use `car::avPlot()`.
    ...
    ```{r}
    #| fig-alt: "Added variable plot of age."
    lm_bp <- lm(sbp ~ age + birthweight, data = bp)
    car::avPlot(model = lm_bp, variable = "age")
    ```
    
    ```{r}
    #| fig-alt: "Added variable plot of birthweight."
...
#| fig-width: 6
#| fig-height: 6
#| fig-alt: "All added variable plots."
car::avPlots(lm_body)
```
...
```{r}
#| fig-alt: "Scatterplot of FEV on Age with OLS line."
aout <- augment(lmout)
...
```{r}
#| fig-alt: "Boxplot of FEV on smoking status."
...
#| fig-width: 4
#| fig-height: 4
#| fig-alt: "Added variable plot of smoking status."
car::avPlot(lmout, variable = "Smokenon-current")
```
```{r}
#| fig-width: 4
#| fig-height: 4
#| fig-alt: "Added variable plot of sex."
