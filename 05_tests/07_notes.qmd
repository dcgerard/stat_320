...
-   We want to fix Type I error at level $\alpha$:
    ...
    plt_t(ub = crit, df = df, rng = c(-4, 4)) +
      ...
      annotate(geom = "text", x = crit - 0.4, y = 0.02, label = expression(alpha), size = 7) +
...
-   Large values of $z$ indicate support that $p$ is larger than $p_0$, so the $p$-value is the probability (under $H_0$) of being larger than what we saw:
    ...
    plt_t(mu = 0, lb = 1.5) +
      ...
      geom_vline(xintercept = 1.5, linetype = "dashed", color = "blue", size = 1) +
...
-   Two-tailed p-value:
    ...
    plt_t(lb = 1.5, two_sided = TRUE) +
      ...
      geom_vline(xintercept = c(-1.5, 1.5), linetype = "dashed", color = "blue", size = 1) +
      theme(axis.text.x = element_text(size = 15))
    ```
...
- $H_1: p < p_0$
  ...
  - Graphic:
    ...
    plt_t(mu = 0, ub = -1.5) +
      ...
      geom_vline(xintercept = -2, linetype = "dashed", color = "blue", size = 1) +
...
- $H_1: p \neq p_0$
  ...
  - Graphic:
    ...
    plt_t(lb = 1.5, two_sided = TRUE) +
      ...
      geom_vline(xintercept = c(-1.5, 1.5), linetype = "dashed", color = "red", size = 1) +
      geom_vline(xintercept = 2, linetype = "dashed", color = "blue", size = 1) +
      theme(axis.text.x = element_text(size = 15, angle = 90, hjust = 1, vjust = 0.5),
            axis.text.y = element_blank(),
            axis.ticks.y = element_blank())
