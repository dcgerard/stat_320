...
![](./05_figs/hypothesis_tests.png){alt="Visual of observed data versus hypothetical data under null with tail probability marked"}\ 
...
-   We want to fix Type I error at level $\alpha$:
    ...
    #| echo: false
    #| warning: false
    #| fig-alt: "Density plot demonstrating Type I error is alpha at a given threshold."
    alpha <- 0.05
    df <- 9
    crit <- qt(alpha, df)
    
    plt_t(ub = crit, df = df, rng = c(-4, 4)) +
      theme(axis.title = element_text(size = 17)) +
      ylab("Density") +
      xlab("t") +
      scale_x_continuous(breaks = crit, labels = "a") +
      geom_vline(xintercept = crit, linetype = "dashed") +
      annotate(geom = "text", x = crit - 0.4, y = 0.02, label = alpha, size = 7, parse = TRUE) +
      ggtitle(expression(t[n-1]-Distribution)) +
...
-   Example: Let's choose $\alpha = 0.01$ and run the test for the birthweight example
    ...
    #| warning: false
    #| fig-width: 5
    #| fig-alt: "Density plot showing our observed value is not beyond critical value."
    alpha <- 0.01
    df <- 99
    t_obs <- -2.08
    crit <- qt(alpha, df)
...
-   In the one-sided hypothesis test case, the $p$-value is the probability of being to the left of the observed value.
    ...
    #| echo: false
    #| fig-width: 5
    #| fig-alt: "Density plot showing p-value is greater than alpha when observed value is not beyond critical value."
    alpha <- 0.01
    df <- 99
    t_obs <- -2.08
    crit <- qt(alpha, df)
    
    plt_t( df = df, ub = t_obs) +
      theme(axis.title = element_text()) +
      geom_vline(xintercept = crit, linetype = "dashed") +
      geom_vline(xintercept = t_obs, color = "blue", linewidth = 1) +
      annotate("text", x = crit - 0.15, y = 0.2, label = "Critical", size = 4, angle = 90) +
...
-   What constitutes "more weird" in this case is the area in both tails.
    ...
    df <- 10
    t_obs <- 1.5
    plt_t(df = df, lb = t_obs, two_sided = TRUE) +
      theme(axis.title = element_text()) +
      geom_vline(xintercept = t_obs, color = "blue", linewidth = 1) +
      annotate("text", x = t_obs + 0.15, y = 0.2, label = "Observed", size = 4, color = "blue", angle = 90) +
...
-   This is what $\Phi()$ looks like
    ```{r}
    #| echo: false
    #| fig-alt: "Line plot of CDF of standard normal."
...
-   Large values of $z$ indicate support that $p$ is larger than $p_0$, so the $p$-value is the probability (under $H_0$) of being larger than what we saw:
    ...
    plt_t(mu = 0, lb = 1.5) +
      ...
      geom_vline(xintercept = 1.5, linetype = "dashed", color = "blue", linewidth = 1) +
...
-   Two-tailed p-value:
    ...
    plt_t(lb = 1.5, two_sided = TRUE) +
      ...
      geom_vline(xintercept = c(-1.5, 1.5), linetype = "dashed", color = "blue", linewidth = 1) +
      theme(axis.text.x = element_text(size = 15))
    ```
...
- $H_1: p \neq p_0$
  ...
  - Graphic:
    ...
    plt_t(lb = 1.5, two_sided = TRUE) +
      ...
      geom_vline(xintercept = c(-1.5, 1.5), linetype = "dashed", color = "red", linewidth = 1) +
      geom_vline(xintercept = 2, linetype = "dashed", color = "blue", linewidth = 1) +
      theme(axis.text.x = element_text(size = 15, angle = 90, hjust = 1, vjust = 0.5),
            axis.text.y = element_blank(),
            axis.ticks.y = element_blank())
...
-   E.g., $X = 3$, $H_0: p_0 = 0.5$, $H_1: p < 0.5$. This the the PMF of $X$ under $H_0$ and the sum of the orange bars is the $p$-value.
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is sum of mass less than or equal to our observed value."
    tibble(x = 0:10) |>
...
-   E.g., $X = 3$, $H_0: p_0 = 0.5$, $H_1: p_1 > 0.5$. This the the PMF of $X$ under $H_0$ and the sum of the orange bars is the $p$-value.
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is sum of mass greater than or equal to our observed value."
    tibble(x = 0:10) |>
      mutate(y = dbinom(x = x, size = 10, prob = 0.5),
             add = x < 3) |>
      ggplot(aes(x = x, xend = x, yend = y, color = add)) +
...
-   E.g., $X = 3$, $H_0: p_0 = 0.5$, $H_1: p < 0.5$. This the the PMF of $X$ under $H_0$ and the two times the sum of the orange bars is the $p$-value.
    ...
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is twice mass less than or equal to our observed value."
    tibble(x = 0:10) |>
      mutate(y = dbinom(x = x, size = 10, prob = 0.5),
             add = x <= 3) |>
      ggplot(aes(x = x, xend = x, yend = y, color = add)) +
...
- It's very similar to the normal power calculations. E.g., here is a visual of the power calculation when $H_1: p < p_0$:
  ...
  ![](./05_figs/power_binom_test.png){alt="Distribution of sample proportion under null and alternative demonstrating power calculation."}\ 
...
-   E.g., we observe $x = 3$ and our hypotheses are $H_0: \mu = 5$  and $H_1: \mu < 5$. Then the $p$-value is the probability of being less than or equal to 3 given $\mu = 5$ (sum of orange bars).
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is mass less than or equal to our observed value."
    tibble(x = 0:15) |>
...
-   E.g., we observe $x = 3$ and our hypotheses are $H_0: \mu = 5$  and $H_1: \mu > 5$. Then the $p$-value is the probability of being greater than or equal to 3 given $\mu = 5$ (sum of orange bars).
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is mass greater than or equal to our observed value."
    tibble(x = 0:15) |>
      mutate(y = dpois(x = x, lambda = 5),
             add = x < 3) |>
      ggplot(aes(x = x, xend = x, yend = y, color = add)) +
...
-   E.g., we observe $x = 3$ and our hypotheses are $H_0: \mu = 5$  and $H_1: \mu \neq 5$. Then the $p$-value is sum of all probabilities less than that of $3$ (sum of orange bars).
    ```{r}
    #| echo: false
    #| fig-alt: "P-value is sum of mass which is as or less probable than our observed value."
    tibble(x = 0:15) |>
      mutate(y = dpois(x = x, lambda = 5),
...
```{r}
#| echo: false
#| fig-alt: "Density plot showing p-value is twice area to the right of absolute value of observed value."
plt_t(lb = 1.732) +
  annotate(geom = "text", x = 2.5, y = 0.1, label = "x2", size = 15, color = "#E69F00") +
...
```{r}
#| echo: false
#| fig-alt: "Density plot showing p-value is twice area to the right of observed value."
plt_t(lb = 1.52, df = 6) +
  annotate(geom = "text", x = 2.5, y = 0.1, label = "x2", size = 15, color = "#E69F00") +
  geom_vline(xintercept = 1.52, linetype = "dashed", size = 1) +
...
Either way, we fail to reject the null and say that we do not have enough evidence to conclude that these folks have different rates of cataracts than the general population.
:::
...
Either way, we reject $H_0$ and conclude that this population has different rates of cataracts than the general population.
:::
